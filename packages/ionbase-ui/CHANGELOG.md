...
Dev Mode's code panel is paywalled. The component **description** field is not,
and both Dev Mode and the Figma MCP read tools surface it — so all 28 mapped
components now carry a generated block naming the import, a working snippet,
every Figma property mapped to its real prop, and a link to the full contract.

Anyone opening the IonBase library, on any plan, now sees
`<Button variant="primary-brand" size="sm">Label</Button>` rather than having to
guess. That was the last thing Code Connect gave that this repo did not.

Generated by `scripts/build-figma-descriptions.mjs` from the verified
`figma-map.json`, and applied with `figma/apply-descriptions.js`. Nothing is
authored, so nothing can disagree with the code. The block is fenced by markers,
so re-running replaces only the block and leaves the hand-written description
untouched.

### Fixed — a Figma API trap, recorded so it is not repeated

`node.description` HTML-escapes on write. 16 characters in, 40 out, and every
write stacks another layer — enough to corrupt an apostrophe a designer typed by
hand. Seven descriptions were damaged on the first pass and fully restored;
`node.descriptionMarkdown` round-trips exactly and is now what the script uses.

Caught by arithmetic, not by eye: the stored length was 3055 where the text was 3011.

**Minor, not patch**: new generated artefact, no behaviour change.
